...
export const EDUCATION_PROJECTS: Project[] = [
  // ─── Higher School of Economics — Bachelor's in Software Engineering ──────
  {
    ...
    description: {
      ...
      tech: {
        en: "ASP.NET MVC backend on .NET Framework. The browser grabs a webcam frame over WebRTC and posts it to the server, which calls Azure's Face API to detect and match it, storing portraits in Azure Blob Storage. The C# refresher gained here paid off directly: it's what let him walk into the OZON backend internship interview with confidence.",
        de: "ASP.NET-MVC-Backend auf dem .NET Framework. Der Browser erfasst per WebRTC ein Webcam-Bild und sendet es an den Server, der über Azures Face API Gesichter erkennt und abgleicht; Porträts landen in Azure Blob Storage. Die hier aufgefrischten C#-Kenntnisse zahlten sich direkt aus: Sie gaben ihm die Sicherheit für das Vorstellungsgespräch zum Backend-Praktikum bei OZON.",
        ru: "Бэкенд на ASP.NET MVC (.NET Framework). Браузер захватывает кадр с веб-камеры через WebRTC и отправляет на сервер, который через Azure Face API распознаёт и сверяет лицо, а портреты хранит в Azure Blob Storage. Освежённые здесь знания C# напрямую окупились: именно они позволили уверенно пройти собеседование на бэкенд-стажировку в OZON.",
      },
    },
  ...
  },
  {
    ...
    category: "education",
    tagline: {
      en: "Arduino robot that identifies 13 colors and responds with sound",
      de: "Arduino-Roboter, der 13 Farben erkennt und mit Klang reagiert",
      ru: "Arduino-робот, распознающий 13 цветов и отвечающий звуком",
    },
    description: {
      hr: {
      ...
      tech: {
        en: "Arduino Uno reading a TCS3200 color sensor, classifying readings into 13 named colors and mapping each to a distinct buzzer tone, with an LED fallback for anything it can't recognize.",
        de: "Ein Arduino Uno liest einen TCS3200-Farbsensor aus, ordnet die Werte 13 benannten Farben zu und spielt für jede einen eigenen Summer-Ton — bei nicht erkannten Farben springt eine LED ein.",
        ru: "Arduino Uno считывает датчик цвета TCS3200, распознаёт 13 именованных цветов и воспроизводит для каждого свой тон на зуммере; для нераспознанных цветов включается LED.",
      },
    },
  ...
  },
  {
    ...
    description: {
      ...
      tech: {
        en: "Backend in Java Spring Boot — JWT auth, WebSocket-based chat, PostgreSQL via Hibernate. Admin panel in React (Vite): film catalog, user management, a stats dashboard with Nivo charts.",
        de: "Backend in Java Spring Boot — JWT-Auth, WebSocket-Chat, PostgreSQL über Hibernate. Admin-Panel in React (Vite): Filmkatalog, Nutzerverwaltung, Statistik-Dashboard mit Nivo-Charts.",
        ru: "Бэкенд на Java Spring Boot — JWT-аутентификация, чат через WebSocket, PostgreSQL через Hibernate. Админка на React (Vite): каталог фильмов, управление пользователями, дашборд статистики на Nivo.",
      },
    },
  ...
  },
  {
    ...
    description: {
      ...
      tech: {
        en: "Next.js/TypeScript, built on React Flow for the ER canvas. Table structure generates live SQL CREATE TABLE statements in an embedded editor — the one part of the \"zero-code\" idea that actually worked. Relation SQL and mock-data generation stayed hardcoded stubs. The most finished piece: a request-tree panel that exports a real Postman collection.",
        de: "Next.js/TypeScript, aufgebaut auf React Flow für die ER-Canvas. Die Tabellenstruktur erzeugt live SQL-CREATE-TABLE-Anweisungen in einem eingebetteten Editor — der einzige Teil der „Zero-Code“-Idee, der tatsächlich funktionierte. SQL für Relationen und Mock-Daten blieben hartcodierte Platzhalter. Am weitesten fertig: ein Request-Baum-Panel, das eine echte Postman-Collection exportiert.",
        ru: "Next.js/TypeScript на React Flow для ER-канвы. Структура таблиц вживую генерирует SQL-запросы CREATE TABLE во встроенном редакторе — единственная часть идеи «zero-code», которая реально заработала. SQL для связей и генерация тестовых данных остались захардкоженными заглушками. Самая законченная часть — панель дерева запросов, экспортирующая настоящую коллекцию Postman.",
      },
    },
    stack: ["Next.js", "TypeScript", "React Flow"],
    links: [
  ...
  },
  {
    ...
    description: {
      ...
      tech: {
        en: "Forked the official Docusaurus site and translated the current doc set — API reference, introduction, usage guide, troubleshooting, tutorials — leaving the English source commented above each paragraph as a reference. Deployed to a custom js.org subdomain via gh-pages.",
        de: "Die offizielle Docusaurus-Site geforkt und den aktuellen Dokumentationsstand übersetzt — API-Referenz, Einführung, Nutzung, Troubleshooting, Tutorials —, wobei der englische Originaltext als Referenz über jedem Absatz auskommentiert blieb. Über gh-pages auf eine eigene js.org-Subdomain deployt.",
        ru: "Форкнул официальный сайт на Docusaurus и перевёл актуальный набор документации — API, введение, руководство по использованию, устранение неполадок, туториалы, — оставляя английский оригинал закомментированным над каждым абзацем как референс. Опубликовано на отдельном поддомене js.org через gh-pages.",
      },
    },
  ...
  },
  {
    ...
    description: {
      hr: {
        en: "Bachelor's thesis project — a news aggregator built and shipped in roughly a month of focused work, backed by a real production-style setup, with a companion admin panel for managing RSS sources and publishers.",
        de: "Bachelorarbeitsprojekt — ein News-Aggregator, gebaut und ausgeliefert in rund einem Monat fokussierter Arbeit, mit einem echten produktionsnahen Setup und einem begleitenden Admin-Panel zur Verwaltung von RSS-Quellen und Publishern.",
        ru: "Проект бакалаврской работы — агрегатор новостей, собранный и выпущенный примерно за месяц сфокусированной работы, с настоящей production-инфраструктурой и админкой для управления RSS-источниками и издателями.",
      },
      business: {
      ...
      tech: {
        en: "Go backend (his first project in the language, built with Gin) — PostgreSQL as the source of truth for publishers and RSS sources, ElasticSearch for article search and autocomplete, gocron for scheduled RSS harvesting. Next.js/React frontend (Mantine UI, Redux Toolkit/RTK Query, Yandex Maps) deployed on Vercel, plus a separate admin panel for source management. Full observability: logs in ELK, tracing in Jaeger, metrics in Prometheus/Grafana, Yandex.Metrica for traffic. CI/CD deploys to a rented Docker host on every push to main.",
        de: "Go-Backend (sein erstes Projekt in dieser Sprache, mit Gin) — PostgreSQL als Datenquelle für Publisher und RSS-Quellen, ElasticSearch für Artikelsuche und Autocomplete, gocron für den geplanten RSS-Abruf. Next.js/React-Frontend (Mantine UI, Redux Toolkit/RTK Query, Yandex Maps), deployt auf Vercel, plus separates Admin-Panel zur Quellenverwaltung. Vollständige Observability: Logs in ELK, Tracing in Jaeger, Metriken in Prometheus/Grafana, Yandex.Metrica für Traffic. CI/CD deployt bei jedem Push auf main automatisch auf einen gemieteten Docker-Host.",
        ru: "Бэкенд на Go (первый проект на этом языке, на Gin) — PostgreSQL как источник истины для издателей и RSS-источников, ElasticSearch для поиска по статьям и автодополнения, gocron для планового сбора RSS. Фронтенд на Next.js/React (Mantine UI, Redux Toolkit/RTK Query, Yandex Maps), развёрнут на Vercel, плюс отдельная админка для управления источниками. Полный observability: логи в ELK, трейсинг в Jaeger, метрики в Prometheus/Grafana, Яндекс.Метрика для трафика. CI/CD автоматически деплоит на арендованный Docker-хост при каждом пуше в main.",
      },
    },
  ...
  // ─── Chemnitz University of Technology — Master's in Web Engineering ──────
  {
    ...
    category: "education",
    tagline: {
      en: "1st-semester project: an interactive Chemnitz city map on real municipal open data",
      de: "Projektaufgabe im 1. Semester: eine interaktive Chemnitz-Stadtkarte auf echten kommunalen Open Data",
      ru: "Проектное задание 1-го семестра: интерактивная карта Хемница на реальных муниципальных открытых данных",
    },
    description: {
      hr: {
        en: "First-semester project at TU Chemnitz with a course partner — an interactive city map, backend and infrastructure his side, the Leaflet-based frontend hers.",
        de: "Projektaufgabe im ersten Semester an der TU Chemnitz mit einem Kurspartner — eine interaktive Stadtkarte; Backend und Infrastruktur sein Teil, das Leaflet-basierte Frontend ihrer.",
        ru: "Проектное задание первого семестра в TU Chemnitz вместе с напарницей по курсу — интерактивная карта города: бэкенд и инфраструктура на нём, фронтенд на Leaflet — на ней.",
      },
      business: {
        en: "Academic assignment, no commercial output. Built on real Chemnitz open data — daycare centers, schools, youth services — with user accounts, routing, and a full observability stack: Prometheus, Grafana, Loki, Jaeger, Sentry.",
        de: "Akademische Aufgabe, kein kommerzieller Output. Aufgebaut auf echten Chemnitzer Open Data — Kindertageseinrichtungen, Schulen, Jugendhilfe — mit Nutzerkonten, Routing und vollständigem Observability-Stack: Prometheus, Grafana, Loki, Jaeger, Sentry.",
        ru: "Академическое задание без коммерческого результата. Построено на реальных открытых данных Хемница — детские сады, школы, молодёжные службы — с учётными записями пользователей, построением маршрутов и полным стеком observability: Prometheus, Grafana, Loki, Jaeger, Sentry.",
      },
      tech: {
        en: "Go backend (Gin, MongoDB, JWT + Google OAuth2, Swagger docs) and a React/Vite frontend (Leaflet with routing, TanStack Query, Formik/Yup, Tailwind). Nominatim for geocoding, Docker Compose and Kubernetes manifests for deployment.",
        de: "Go-Backend (Gin, MongoDB, JWT + Google OAuth2, Swagger-Doku) und ein React/Vite-Frontend (Leaflet mit Routing, TanStack Query, Formik/Yup, Tailwind). Nominatim fürs Geocoding, Deployment über Docker Compose und Kubernetes-Manifeste.",
        ru: "Бэкенд на Go (Gin, MongoDB, JWT + Google OAuth2, документация Swagger) и фронтенд на React/Vite (Leaflet с построением маршрутов, TanStack Query, Formik/Yup, Tailwind). Nominatim для геокодинга, деплой через Docker Compose и манифесты Kubernetes.",
      },
    },
    stack: ["Go", "TypeScript", "React", "MongoDB", "Leaflet"],
    links: [
  ...
  },
  {
    ...
    description: {
      ...
      tech: {
        en: "Next.js/TypeScript, the same React Flow + Ace-editor approach as the earlier Lightning project — an ER diagram generates live SQL CREATE TABLE text. No database connection: it outputs SQL, it doesn't run it. Deployed on Vercel.",
        de: "Next.js/TypeScript, derselbe Ansatz aus React Flow und Ace-Editor wie beim früheren Lightning-Projekt — ein ER-Diagramm erzeugt live SQL-CREATE-TABLE-Text. Keine Datenbankverbindung: Es gibt SQL aus, führt es aber nicht aus. Deployt auf Vercel.",
        ru: "Next.js/TypeScript, тот же подход на React Flow и Ace-редакторе, что и в более раннем проекте Lightning — ER-диаграмма вживую генерирует текст SQL CREATE TABLE. Без подключения к базе: инструмент выводит SQL, но не выполняет его. Развёрнуто на Vercel.",
      },
    },
    stack: ["Next.js", "TypeScript", "React Flow"],
    links: [
  ...
  },
  {
    ...
    category: "education",
    tagline: {
      en: "Weekly exercises for Software Service Engineering: REST, raw sockets, SOAP, integration patterns",
      de: "Wöchentliche Übungen für Software Service Engineering: REST, Raw Sockets, SOAP, Integrationsmuster",
      ru: "Еженедельные упражнения по Software Service Engineering: REST, «сырые» сокеты, SOAP, паттерны интеграции",
    },
    description: {
      hr: {
        en: "Weekly coursework for the Software Service Engineering module — around 15 small exercises spanning REST API design, a hand-rolled HTTP server, a manual SOAP client, and classic integration/design patterns.",
        de: "Wöchentliche Kursarbeit für das Modul Software Service Engineering — rund 15 kleine Übungen von REST-API-Design über einen selbstgebauten HTTP-Server bis zu einem manuellen SOAP-Client und klassischen Integrations-/Entwurfsmustern.",
        ru: "Еженедельная курсовая по модулю Software Service Engineering — около 15 небольших упражнений: от проектирования REST API до самодельного HTTP-сервера, ручного SOAP-клиента и классических паттернов интеграции.",
      },
      business: {
      ...
      tech: {
        en: "~15 standalone ASP.NET Core / C# exercises: REST APIs with versioning (a bookmark manager, a books service, both persisting to XML), a hand-rolled HTTP server built directly on TCP sockets, a manual SOAP 1.2 client with hand-built XML envelopes, a Broker enterprise-integration-pattern demo, and a Proxy-pattern exercise with xUnit/Moq tests.",
        de: "~15 eigenständige ASP.NET-Core-/C#-Übungen: REST-APIs mit Versionierung (ein Bookmark-Manager, ein Books-Service, beide mit XML-Persistenz), ein von Hand auf TCP-Sockets aufgebauter HTTP-Server, ein manueller SOAP-1.2-Client mit handgebauten XML-Envelopes, eine Broker-Enterprise-Integration-Pattern-Demo und eine Proxy-Pattern-Übung mit xUnit/Moq-Tests.",
        ru: "~15 отдельных упражнений на ASP.NET Core / C#: REST API с версионированием (менеджер закладок, сервис книг — оба хранят данные в XML), HTTP-сервер, написанный вручную поверх TCP-сокетов, ручной SOAP 1.2-клиент с самодельными XML-конвертами, демо паттерна Broker (enterprise integration) и упражнение на паттерн Proxy с тестами на xUnit/Moq.",
      },
    },
  ...
  },
  {
    ...
    description: {
      ...
      tech: {
        en: "4 Python/FastAPI services: an orchestrator, four embedding-model containers (BAAI bge-m3, multilingual-e5-base, jina-embeddings-v3, LaBSE), and a reranker (jina-reranker-v3) — behind a Vite/React frontend that streams the search pipeline live over Server-Sent Events. Qdrant runs hybrid dense+sparse search via reciprocal rank fusion; MongoDB holds the rest. The thesis's core research contribution is the retrieval pipeline design and its benchmark methodology — weighted relevance scored across models and search modes — rather than a custom-trained reranker.",
        de: "4 Python-/FastAPI-Services: ein Orchestrator, vier Embedding-Modell-Container (BAAI bge-m3, multilingual-e5-base, jina-embeddings-v3, LaBSE) und ein Reranker (jina-reranker-v3) — dahinter ein Vite/React-Frontend, das die Suchpipeline live per Server-Sent Events streamt. Qdrant führt hybride Dense-plus-Sparse-Suche per Reciprocal Rank Fusion aus; MongoDB übernimmt den Rest. Der Forschungsbeitrag der Arbeit liegt im Design der Retrieval-Pipeline und ihrer Benchmark-Methodik — gewichtete Relevanzbewertung über Modelle und Suchmodi hinweg — nicht in einem selbst trainierten Reranker.",
        ru: "4 сервиса на Python/FastAPI: оркестратор, четыре контейнера с моделями эмбеддингов (BAAI bge-m3, multilingual-e5-base, jina-embeddings-v3, LaBSE) и реранкер (jina-reranker-v3) — за фронтендом на Vite/React, который вживую стримит пайплайн поиска через Server-Sent Events. Qdrant выполняет гибридный dense+sparse поиск через Reciprocal Rank Fusion; MongoDB хранит остальное. Основной исследовательский вклад работы — в дизайне retrieval-пайплайна и методике бенчмарка (взвешенная оценка релевантности по моделям и режимам поиска), а не в дообученном реранкере.",
      },
    },
    impact: [
      { label: { en: "Services", de: "Services", ru: "Сервисов" }, value: "4" },
      { label: { en: "Embedding models benchmarked", de: "Verglichene Embedding-Modelle", ru: "Сравнено моделей эмбеддингов" }, value: "4" },
    ],
