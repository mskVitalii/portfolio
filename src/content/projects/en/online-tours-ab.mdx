...
**Situation:** onlineTours' growth team had hypotheses about SEO landing pages and booking-flow UX but no reliable way to test them — the frontend was a mix of legacy React and Rails-rendered HTML, with no component library and no A/B testing infrastructure.

**Task:** Give the team both a modern, reusable component library and a proper server-side A/B testing pipeline, so experiments could ship without flicker or engineering babysitting every test.

**Action:** Built a React + TypeScript component library (documented in Storybook, A/B-variant-aware by design) alongside server-side experiment assignment in Ruby on Rails, backed by Redis for fast, deterministic variant reads on every page load.

**Result:** The highest-impact experiment — redesigning the tour listing page's price display — delivered an 11.63% uplift on the primary booking conversion metric over a 4-week test window.
